...
// GET /history
app.get("/history", (c) => {
  return htmlResponse(HTML_HISTORY);
});

// GET /view/:id
app.get("/view/:id", (c) => {
  const id = c.req.param("id");
  const html = HTML_VIEW.replace("__CLIP_ID__", id);
  return htmlResponse(html);
});

// GET /:id (4-char short link → serve view page directly)
app.get("/:id", (c) => {
  const id = c.req.param("id");
...
textInput.addEventListener('input', () => {
  ...
  charCount.textContent = len + ' / ' + kb + 'KB';
...
function copyUrl() {
  navigator.clipboard.writeText(currentUrl).then(() => showToast('Link copied'));
}
...
function getUserId() {
  ...
  if (!uid) {
    ...
    localStorage.setItem('clipboard_uid', uid);
  }
  return uid;
}
${toastScript('')}
...
<div class="container">
  ...
  <div id="expiredBanner" class="expired-banner" style="display:none">
    ⚠ This clip has expired. Content is still viewable.
  </div>
  ...
  <div id="passwordSection">
    ...
    <div style="margin-top:16px">
      <button onclick="copyContent()" id="copyBtn">copy to clipboard</button>
    </div>
  </div>
</div>
...
(async function loadClip() {
  ...
  try {
    const res = await fetch('/api/get/' + CLIP_ID + (password ? '?password=' + encodeURIComponent(password) : ''));
    const data = await res.json();
    ...
    if (res.status === 404) {
      document.getElementById('clipContent').textContent = 'Clip not found';
    ...
    if (!data.expired) {
      ...
      document.getElementById('countdown').textContent = 'expired: ' + formatTime(data.expires_at);
    }
  } catch (e) {
    document.getElementById('clipContent').textContent = 'Failed to load clip';
    document.getElementById('clipContent').style.display = 'block';
  }
})();
...
function verifyPassword() {
  ...
  if (!pw) return;
  location.href = location.pathname + '?password=' + encodeURIComponent(pw);
}

function copyContent() {
  navigator.clipboard.writeText(clipText).then(() => showToast('Copied to clipboard'));
}
...
async function loadHistory() {
  const userId = getUserId();
  if (!userId) {
    document.getElementById('historyList').innerHTML = '<div class="empty-state">No user ID found</div>';
    return;
  }
  ...
  try {
    ...
    if (items.length === 0) {
      list.innerHTML = '<div class="empty-state">No clips found</div>';
    ...
    list.innerHTML = items.map(item => {
      ...
      return '<li class="history-item">' +
        '<span class="preview"><a class="preview-link" href="/view/' + item.id + '">' +
        '<span class="preview-text">' + escaped + '</span>' +
        '</a><span class="badge ' + (item.expired ? 'expired' : 'active') + '">' +
        (item.expired ? 'expired' : 'active') + '</span></span>' +
        '<div class="meta-info">' + formatTime(item.created_at) + '</div>' +
        '</li>';
    }).join('');
    ...
    pag.innerHTML = '';
    if (currentPage > 1) {
      pag.innerHTML += '<button onclick="goPage(' + (currentPage - 1) + ')">prev</button>';
    }
    pag.innerHTML += '<span style="color:var(--text-dim);padding:8px">page ' + currentPage + '</span>';
    if (data.hasMore) {
      pag.innerHTML += '<button onclick="goPage(' + (currentPage + 1) + ')">next</button>';
    }
  } catch (e) {
