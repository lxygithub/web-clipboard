...
app.post("/api/create", async (c) => {
  ...
  if (body.text.length > maxSize) {
    return c.json({ error: "Text exceeds 100KB limit" }, 400);
  }

  const validExpirations = [3600, 86400, 604800, 2592000]; // 1h, 24h, 7d, 30d
  if (!validExpirations.includes(body.expiresIn)) {
    return c.json({ error: "Invalid expiration time" }, 400);
  }
...
app.get("/api/get/:id", async (c) => {
  ...
  const isExpired = (clip.expires_at as number) < Math.floor(Date.now() / 1000);
  ...
  return c.json({
    ...
    expired: isExpired,
    hasPassword: !!(clip as any).password,
  });
});

// GET /api/verify-password/:id
app.get("/api/verify-password/:id", async (c) => {
  const id = c.req.param("id");
  const password = c.req.query("password");

  const clip = await c.env.DB.prepare(
    "SELECT password FROM clips WHERE id = ?"
  ).bind(id).first();
  ...
  const correct = (clip as any).password === password;
  return c.json({ valid: correct });
});
